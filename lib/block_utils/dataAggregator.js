    ...
    return Promise.all(promiseArray)
        .then(function(promiseResponse){
            ...
            promiseResponse.forEach(function(data){
                ...
                total_transactions.push({transaction_count :oThis.checkNull(val), timestamp: timestamp, ost_amount: oThis.checkNull(data[0]['SUM(total_transaction_value)']) * contractHash.price});
                ...
                total_transfers.push({transaction_count :oThis.checkNull(val), timestamp: timestamp, ost_amount: oThis.checkNull(data[0]['SUM(total_transfer_value)']) * contractHash.price});

    ...
    .then(function (promiseResponse) {
      ...
      promiseResponse.forEach(function (data) {
        ...
        total_transactions.push({
          ...
          ost_amount: TokenUnits.toBigNumber(data[0]['SUM(total_transaction_value)']).toString()
        });
        ...
        total_transfers.push({
          timestamp: timestamp,
          transaction_count: oThis.checkNull(val),
          ost_amount: TokenUnits.toBigNumber(data[0]['SUM(total_transfer_value)']).div(oThis.checkNull(contractHash.price)).toString()
        });
            ...
            .then(function (noOfRows) {
              oThis.web3Interact.getTotalSupply(event._token.toLowerCase())
                .then(function (totalSupply) {
                  ...
                  dataRow.push(0);
                  dataRow.push(0);
                  dataRow.push(TokenUnits.weiToEther(totalSupply));
                  dataRow.push(null);
                  dataRow.push(null);
                  dataRow.push(null);
                  dataRow.push(null);

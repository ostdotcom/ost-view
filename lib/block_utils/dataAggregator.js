    ...
    .then(function (promiseResponse) {
      ...
      promiseResponse.forEach(function (data) {
        ...
        total_transfers.push({
          timestamp: timestamp,
          transaction_count: oThis.checkNull(val),
          ost_amount: TokenUnits.weiToEther(TokenUnits.toBigNumber(data[0]['SUM(total_transfer_value)']).mul(oThis.checkNull(contractHash.price))).toString()
        });
  ...
  return Promise.all(promiseArray)
    .then(function (result) {
      ...
      data.forEach(function (entity) {
        ...
        entity.market_cap = TokenUnits.toBigNumber(entity.circulation).mul(TokenUnits.toBigNumber(entity.price)).toString(10);
        ind += 1;
      });
      return Promise.resolve(data);
    });
            ...
            .then(function (noOfRows) {
              oThis.web3Interact.getTotalSupply(event._token.toLowerCase())
                .then(function (totalSupply) {
                  ...
                  dataRow.push(
                    TokenUnits.toBigNumber(1)
                      .div(TokenUnits.toBigNumber(event._conversionRate))
                        .mul(TokenUnits.convertToBigNumber(10)
                          .toPower(event._conversionRateDecimals)).toString(10)
                  );
                  dataRow.push(0);
                  dataRow.push(0);
                  dataRow.push(0);

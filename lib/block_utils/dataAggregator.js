    ...
    return oThis.dbInteract.getTransactionType(txnHashes)
        .then(function(transactionsTypes) {
            ...
            internalTransactions.forEach(function(internalTxn) {
                ...
                const totalTransferValue = new BigNumber(obj.total_transfer_value),
                  internalTxnTokens = new BigNumber(internalTxn.tokens),
                  finalTransferValue = totalTransferValue.plus(internalTxnTokens)
                ;

                obj.total_transfers = +obj.total_transfers + 1;
                obj.total_transfer_value = +finalTransferValue;
...
DataAggregator.prototype.insertAddressData = function (transactions, internalTransactions) {
    ...
    internalTransactions.forEach(function(internalTxn) {
        ...
        if (data[internalTxn.t_from][internalTxn.contract_address] == undefined) {
            data[internalTxn.t_from][internalTxn.contract_address] = {tokens: 0, tokens_earned:0 ,tokens_spent:0, transactions: 0};
        }

        var fromObj = data[internalTxn.t_from][internalTxn.contract_address];
        fromObj.tokens = +fromObj.tokens - internalTxn.tokens;
        fromObj.tokens_spent = +fromObj.tokens_spent + internalTxn.tokens;
        ...
        if (data[internalTxn.t_to][internalTxn.contract_address] == undefined) {
            data[internalTxn.t_to][internalTxn.contract_address] = {tokens: 0, tokens_earned:0 ,tokens_spent:0, transactions: 0};
        }
    ...
    return Promise.all(promiseArray)
        .then(function(promiseResponse){
            ...
            promiseResponse.forEach(function(data){
                ...
                total_transactions.push({transaction_count :oThis.checkNull(val), timestamp: timestamp, ost_amount: oThis.checkNull(data[0]['SUM(total_transaction_value)']) * oThis.checkNull(contractHash.price)});
                ...
                const total_transfer_value1 = new BigNumber(oThis.checkNull(data[0]['SUM(total_transfer_value)']))
                  ,price = new BigNumber(oThis.checkNull(contractHash.price))
                  ,finalAmount = total_transfer_value1.mul(price).div(10e+18)
                ;

                total_transfers.push({transaction_count :oThis.checkNull(val), timestamp: timestamp, ost_amount: finalAmount});
                ...
                .then(function(data) {
                    ...
                    if ( 0 < data.length ) {
                        ...
                        data.forEach(function(subData) {
                            transaction_type_data.push({type: subData['transaction_type'], total_transfers: subData['SUM(a.total_transfers)']});
                            i++;
                        });
                    }

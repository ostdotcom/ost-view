    ...
    .then(function (promiseResponse) {
      ...
      promiseResponse.forEach(function (data) {
        ...
        total_transactions.push({
          ...
          ost_amount: oThis.checkNull(data[0]['SUM(total_transaction_value)']) * oThis.checkNull(contractHash.price)
        });
        ...
        total_transfers.push({
          timestamp: timestamp,
          transaction_count: oThis.checkNull(val),
          ost_amount: oThis.checkNull(data[0]['SUM(total_transfer_value)']) * oThis.checkNull(contractHash.price)
        });

...
DataAggregator.prototype.formatAddressData = function (data) {
    ...
    Object.keys(data).forEach(function (key){
        ...
        Object.keys(subData).forEach(function(subkey){
            ...
            var row = [];
            row.push(key);
            row.push(oThis.contractIdMap[subkey]);
            row.push(obj.tokens);
...
DataAggregator.prototype.addGraphForLastDay = function(contractHash, timeId) {
    ...
    return Promise.all(promiseArray)
        .then(function(promiseResponse){
            ...
            var timestamp = timeId - 24 * quantum;
            promiseResponse.forEach(function(data){

                var val = data[0]['SUM(total_transactions)'];
                total_transactions.push({value :oThis.checkNull(val), time: timestamp});

                val = data[0]['SUM(total_transaction_value)'];
                total_transaction_value.push({value :oThis.checkNull(val), time: timestamp});

                val = data[0]['SUM(total_transfers)'];
                total_transfers.push({value :oThis.checkNull(val), time: timestamp});

                val = data[0]['SUM(total_transfer_value)'];
                total_transfer_value.push({value :oThis.checkNull(val), time: timestamp});
...
DataAggregator.prototype.addGraphForLastHour = function(contractHash, timeId) {
    ...
    return Promise.all(promiseArray)
        .then(function(promiseResponse){
            ...
            var total_transfers = [];
            var total_transfer_value = [];
            var timestamp = timeId - 12 * quantum;
            promiseResponse.forEach(function(data){
            ...
            contractHash.tokens_transfer_data.Hour = total_transfer_value;
            if (contractHash.tokens_volume_data == undefined) {
                contractHash.tokens_volume_data = {};
            }
            contractHash.tokens_volume_data.Hour = total_transfers;
            if (contractHash.transactions_data == undefined) {
                contractHash.transactions_data = {};
            }
            contractHash.transactions_data.Hour = total_transaction_value;
            if (contractHash.transactions_volume_data == undefined) {
                contractHash.transactions_volume_data = {};
            }
            contractHash.transactions_volume_data.Hour = total_transactions;
...
DataAggregator.prototype.addGraphForLastWeek = function(contractHash, timeId) {
    ...
    var oThis = this;
    var promiseArray = [];
    var quantum = 12 * 24 * constants.AGGREGATE_CONSTANT;
    var timeId1 = timeId - 7 * quantum;
    var timeId2 = timeId1 + quantum;
    for (var i=0;i<7;i++) {
        promiseArray.push(oThis.dbInteract.getAggregateDataWithinTimestamp(contractHash.id, timeId1, timeId2));
        timeId1 = timeId2;
    ...
    return Promise.all(promiseArray)
        .then(function(promiseResponse){
            ...
            var total_transfers = [];
            var total_transfer_value = [];
            var timestamp = timeId - 7 * quantum;
            promiseResponse.forEach(function(data){
            ...
            contractHash.tokens_transfer_data.Week = total_transfer_value;
            if (contractHash.tokens_volume_data == undefined) {
                contractHash.tokens_volume_data = {};
            }
            contractHash.tokens_volume_data.Week = total_transfers;
            if (contractHash.transactions_data == undefined) {
                contractHash.transactions_data = {};
            }
            contractHash.transactions_data.Week = total_transaction_value;
            if (contractHash.transactions_volume_data == undefined) {
                contractHash.transactions_volume_data = {};
            }
            contractHash.transactions_volume_data.Week = total_transactions;
...
DataAggregator.prototype.addGraphForLastMonth = function(contractHash, timeId) {
    ...
    return Promise.all(promiseArray)
        .then(function(promiseResponse){
            ...
            var total_transfers = [];
            var total_transfer_value = [];
            var timestamp = timeId - 30 * quantum;
            promiseResponse.forEach(function(data){
            ...
            contractHash.tokens_transfer_data.Month = total_transfer_value;
            if (contractHash.tokens_volume_data == undefined) {
                contractHash.tokens_volume_data = {};
            }
            contractHash.tokens_volume_data.Month = total_transfers;
            if (contractHash.transactions_data == undefined) {
                contractHash.transactions_data = {};
            }
            contractHash.transactions_data.Month = total_transaction_value;
            if (contractHash.transactions_volume_data == undefined) {
                contractHash.transactions_volume_data = {};
            }
            contractHash.transactions_volume_data.Month = total_transactions;

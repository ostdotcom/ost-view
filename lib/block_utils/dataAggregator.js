...
DataAggregator.prototype.formatData = function (data) {
  ...
  Object.keys(data).forEach(function (key) {
    ...
    row.push(0); //TODO: will contain token ost volume after calculation
...
DataAggregator.prototype.addGraph = function (params) {
  ...
  var timeId = params.timeId + params.quantum - (params.timeId % params.quantum);
  ...
  return Promise.all(promiseArray)
    .then(function (result) {
      ...
      data.forEach(function (entity) {
        ...
        entity.market_cap = TokenUnits.toBigNumber(entity.total_supply).mul(TokenUnits.toBigNumber(entity.price)).toString(10);
        ind += 1;
      });
      return Promise.resolve(data);
    });

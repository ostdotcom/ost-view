...
/**
 * Insert addresses after looking in cache
 *
...
AddAddressesKlass.prototype = {
  ...
  perform: async function(){
    ...
    for(let key in oThis.addressesHash){
      ...
      if(!cachedAddressData[key] || (Number(cachedAddressData[key].address_type) < Number(addrType))){
    ...
    if(dataToInsert.length > 0){
      await addressObj.insertMultiple(AddressKlass.DATA_SEQUENCE_ARRAY, dataToInsert)
        .onDuplicate('address_type = IF(VALUES(address_type)>address_type, VALUES(address_type), address_type)').fire();

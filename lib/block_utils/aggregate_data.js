  ...
  aggregateAndInsert: async function(allTransactions, tokenTransfersHash, transactionTypeHash, contractAddressIds){
    ...
    for(let txn_id in tokenTransfersHash) {
      ...
      for (let i = 0; i<internalTxArray.length; i++) {
        ...
        const inTxObj = aggregateData[btId][txTypeId];
        ...
        inTxObj.token_ost_volume = TokenUnits.div(inTxObj.total_transfer_value, ostToBtConversationFactor);

        const brandedTokenAgg = brandedTokenAggData[btId] || {}
          , tokenTransfers = brandedTokenAgg.token_transfers || 0
          , tokenOstVolumeHash = brandedTokenAgg.token_ost_volume || {}
        ;

        brandedTokenAgg.token_transfers = TokenUnits.add(tokenTransfers, 1);
        brandedTokenAgg.token_ost_volume = Object.assign(tokenOstVolumeHash, {[txTypeId] :inTxObj.token_ost_volume});
    ...
    if (Object.keys(btIdsDataHash).length > 0){
      ...
      for (let btId in btIdsDataHash){
        ...
        const btData = btIdsDataHash[btId]
          , tokenOstVolumeData = btData.token_ost_volume
        ...
        if (brandedToken){
          ...
          let finaltokenOstVolume = 0;
          for (let key in tokenOstVolumeData){
            const ostVolumeForkey =  tokenOstVolumeData[key];
            finaltokenOstVolume = TokenUnits.add(finaltokenOstVolume,ostVolumeForkey);
          }
          ...
          finalDataToInsert.push([btId, (holderCount || 0), btData.token_transfers, btData.total_supply,
            finaltokenOstVolume, btData.market_cap, (tc.total_tokens || 0)])
        }
      }

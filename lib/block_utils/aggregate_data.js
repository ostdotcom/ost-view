...
AggregateData.prototype = {
  ...
  aggregateAndInsert: async function(allTransactions, tokenTransfersHash, transactionTypeHash, contractAddressIds){
    ...
    while(true) {
      ...
      console.log("batchedContractAddressIds : ", contractAddressIds.length, offset, offset+batchSize);
    ...
    for(let txn_id in tokenTransfersHash) {
      ...
      for (let i = 0; i<internalTxArray.length; i++) {
        ...
        if (!btId) {
          logger.notify("l_du_ag_ai_1","Branded token id missing for token transfer : "+internalTx);
        }
    ...
    const brandedTokenProcessedData = await oThis.proceessBrandedTokenStatsData(brandedTokenAggData, brandedTokens);
    await oThis.updateBrandedTokenStats(brandedTokenProcessedData);
  },


  proceessBrandedTokenStatsData: async function(btIdsDataHash, brandedTokens){
    ...
    if (Object.keys(btIdsDataHash).length > 0){
      ...
      for (let btId in btIdsDataHash){
        ...
        btData['market_cap'] = TokenUnits.toBigNumber(btData.total_supply).div(TokenUnits.toBigNumber(brandedToken.conversion_rate)).toString(10);
  ...
  updateBrandedTokenStats: async function(brandedTokenAggData){
    ...
    if (brandedTokenAggData && brandedTokenAggData.length > 0){
      ...
      console.log("brandedTokenAggData not available");
    }
   },

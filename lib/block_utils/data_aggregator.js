...
DataAggregator.prototype.aggregateData = async function (timeId, aggregateDataCompleteCallback) {
  ...
  const performer = async function() {
    ...
    while(true) {
      ...
      // Get transactions in batch for given timeframe
      const batchedTransactions = await new TransactionModelKlass().select(['transaction_hash','t_from', 't_to', 'tokens', 'contract_address','block_number', 'transaction_index', 'gas_price', 'gas_used', 'nounce', 'timestamp']).where(['timestamp >= ? AND timestamp < ?',
      ...
      // Get branded token transactions for fetched batch
      ...
      // Get transaction types for fetched batch
      const transactionsTypeRecords = await new TransactionTypeModelKlass().select('*')
        .where(['transaction_hash IN (?)', batchedTransactionHashes]).fire();
   ...
   if(allTransactions.length > 0 ){
      await oThis.insertAggregateData(timeId, allTransactions, allTokenTransactions, txHashToTypeIdMap);
 ...
 * @param {array} txHashToTypeIdMap - Tx hash to type map
 * @returns {promise}
 */
DataAggregator.prototype.insertAggregateData = async function (timeId, transactions, internalTransactions, txHashToTypeIdMap) {
  const oThis = this
    , aggregateData = {}
    //, txnHashes = []
    //, txHashToTypeIdMap = {}
 ...
 * @returns {Promise}
 */
DataAggregator.prototype.insertAddressData = async function (transactions, internalTransactions) {
  var oThis = this;
  const data = {};
  ...
  const updatedData = await  oThis.updateAddressBalance(data);

  var formattedData = oThis.formatAddressData(updatedData);

  logger.log("Total data to be inserted in address table length-", formattedData.length);
  ...
  while(true){
    ...
    const batchedData = formattedData.slice(offset, (offset+limit) );
    if ( !batchedData.length ) {
      break;
    }
    await oThis.dbInteract.insertOrUpdateAddressData( batchedData ).then(function ( i_d_k ) {
        logger.log("insertOrUpdateAddressData done :: offset:", offset, "limit:", limit);
        return i_d_k;
      }).catch( function ( reason ) {
        logger.error("insertOrUpdateAddressData FAILED :: offset:", offset, "limit:", limit, "reason :: ", reason );
        logger.error("batchedData", batchedData);
      })
    ;
    offset += limit;

 ...
 * @param logArray
 */
TransactionLogProcessor.prototype.getLogsDecodedArray = function (logArray) {
  for (let ind in logArray) {
    const logs = logArray[ind].logs;
    logArray[ind].logs = ercToken.decodeLogs(logs);
  }
  return logArray;
};
/**
 ...
 * @returns {Promise}
 */
TransactionLogProcessor.prototype.process = async function (transactionArray) {
  const oThis = this;
  try {
    ...
    logger.error("TransactionLogProcessor :: process :: try catch", err);
    return false;
  }
  return true;
};
 ...
 * @return {Promise}
 */
TransactionLogProcessor.prototype.writeTokenTransactionToDB = function (transactionArray) {
 ...
 * @return {Array}
 */
TransactionLogProcessor.prototype.formatTokenTransactionData = function (transaction, decodedContTransactionList) {
...
TransactionLogProcessor.prototype.insertRegisteredBrandedTokens = async function (decodedTransactionArray) {
  ...
  try {
    while (true) {
      ...
      await new AddressKlass(oThis.chainId).insertMultiple(['address_hash'], addressSubArray, {insertWithIgnore: true}).fire();
  ...
  for (let ind in brandedTokenDetailsArray) {
    ...
    dataRow.push(null);
    dataRow.push(registerEvent.timestamp);
 ...
 * @param addressIdHash
 */
TransactionLogProcessor.prototype.formatTransferData = function (decodedTransactionLogArray, txnIdHash, addressIdHash) {
  const transferArray = [];
  ...
  for (let ind in decodedTransactionLogArray) {
    ...
    for (let no in txn.logs['Transfer']) {
      ...
      formattedTxn.push(addressIdHash[transfer.address]);
      formattedTxn.push(addressIdHash[transfer._from]);
      formattedTxn.push(addressIdHash[transfer._to]);
 ...
 * @param addressIdHash
 */
TransactionLogProcessor.prototype.formatAddressTransferData = function (decodedTransactionLogArray, txnIdHash, addressIdHash) {
  const oThis = this
  ...
  for (let ind in decodedTransactionLogArray) {
    ...
    for (let no in txn.logs['Transfer']) {
      ...
      addressTxnFirst.push(addressIdHash[transfer._from]);
      addressTxnSecond.push(addressIdHash[transfer._to]);
      ...
      addressTxnFirst.push(txnIdHash[transfer.address]);
      addressTxnSecond.push(txnIdHash[transfer.address]);
...
TransactionLogProcessor.prototype.processTransfersWithIds = async function (decodedTransactionLogArray) {
  ...
  for(let ind in decodedTransactionLogArray) {
    const transaction = decodedTransactionLogArray[ind];
    txnHashesArray.push([transaction.hash]);
    const transferArray = transaction.logs['Transfer'];
    for (let no in transferArray) {
      const transferEvent = transferArray[no];
      transferEvent._from && addressesHashesArray.push([transferEvent._from]);
      transferEvent._to && addressesHashesArray.push([transferEvent._to]);
      transferEvent.address && addressesHashesArray.push([transferEvent.address]);
    }
  }
  ...
  try {
    while (true) {
      ...
      await new TransactionHashKlass(oThis.chainId).insertMultiple(['transaction_hash'], txnSubArray, {insertWithIgnore: true}).fire();
    ...
    logger.error("TransactionLogProcessor :: processTransfersWithIds :: txnHashesArray :: try catch :: ", err);
    return Promise.reject(err);
  }
  ...
  try {
    ...
    logger.error("TransactionLogProcessor :: processTransfersWithIds :: addressesHashesArray :: try catch :: ", err);
    return Promise.reject(err);
  }

  const formattedTransferDataArray = oThis.formatTransferData(decodedTransactionLogArray, txnIdHash, addressIdHash);

  const formattedAddressTransferDataArray = oThis.formatAddressTransferData(decodedTransactionLogArray, txnIdHash, addressIdHash);

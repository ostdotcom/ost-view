...
TransactionLogProcessor.prototype.processTransfersWithIds = async function (decodedTransactionLogArray) {
  ...
  for(let ind in decodedTransactionLogArray) {
    ...
    for (let no in transferArray) {
      ...
      transferEvent._from && addressesHashesArray.push([transferEvent._from, addressObject.invertedAddressTypes[addressConst.contractAddress]]);
      transferEvent._to && addressesHashesArray.push([transferEvent._to, addressObject.invertedAddressTypes[addressConst.contractAddress]]);
      transferEvent.address && addressesHashesArray.push([transferEvent.address, addressObject.invertedAddressTypes[addressConst.erc20Address]]);
    }
  }

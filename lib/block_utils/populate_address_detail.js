...
PopulateAddressDetail.prototype.processData = function (allTransactions, allTokenTransfers) {
  const oThis = this
    , successTransactionStatus =  Number(new TransactionKlass(oThis.chainId).invertedTransactionStatus[transactionConst.succeeded])
    , addressData = {}
  ;
...
PopulateAddressDetail.prototype.getBalanceFromGeth = async function (batchedData) {
  ...
  const selectAddressHashesResponse = await addressObj.select(['id', 'address_hash']).where({id: addressIds}).fire();

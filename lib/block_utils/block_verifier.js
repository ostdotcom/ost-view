...
BlockVerifier.prototype.deleteAllDataForBlock = function (blockNumber) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    transactionObj.select(['transaction_hash_id']).where({block_number: blockNumber}).fire()
      .then(function (queryResponseRows) {
...
/*
  BlockVerifier = require('./lib/block_utils/block_verifier.js');
  BlockVerifier.newInstance(1409).deleteAllDataForBlock(0).then(console.log);
 */
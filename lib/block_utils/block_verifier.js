...
BlockVerifier.prototype.verifyBlock = function (blockNumber, verificationCompleteCallback) {
  ...
  oThis.correctBlockInconsistency(blockNumber)
    ...
    .catch(async function (err) {
      ...
      await oThis.dbInteract.updateVerifiedFlag(blockNumber, blockObj.invertedVerified[blockConst.failed]);
      logger.notify('bv_1', 'Block Fetch unsuccesful. exception in verifier', err);
      process.exit(1);
    });
};
//
    ...
    oThis.deleteAllDataForBlock(blockNumber)
      .then(function () {
        ...
        blockFetcher.fetchAndUpdateBlock(blockNumber, function (res) {
          resolve(res);
          // oThis.dbInteract.updateVerifiedFlag(blockNumber, 1)
          //   .then(function (response) {
          //     resolve(response);
          //   });
        });

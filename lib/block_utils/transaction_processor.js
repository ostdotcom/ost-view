...
TransactionProcessor.prototype.writeTransactionsToDB = async function (transactionsArray) {
  ...
  // 100 batch iterate and use createTxInsertPromise
...
TransactionProcessor.prototype.formatExtendedTransactionData = function (transactionArray, txnIdHash) {
  ...
  for (let ind in transactionArray) {
    const txn = transactionArray[ind];
    const formattedTxn = [];
    formattedTxn.push(txnIdHash[txn.hash.toLowerCase()]);
...
TransactionProcessor.prototype.formatAddressTransactionData = function (transactionArray, txnIdHash, addressIdHash) {
  ...
  for (let ind in transactionArray) {
    ...
    addressTxnFirst.push(addressIdHash[txn.from.toLowerCase()]);
    addressTxnSecond.push(addressIdHash[txn.to.toLowerCase()]);

    addressTxnFirst.push(addressIdHash[txn.to.toLowerCase()]);
    addressTxnSecond.push(addressIdHash[txn.from.toLowerCase()]);

    addressTxnFirst.push(txnIdHash[txn.hash.toLowerCase()]);
    addressTxnSecond.push(txnIdHash[txn.hash.toLowerCase()]);
...
TransactionProcessor.prototype.processTransactionsWithIds = async function (transactionArray) {
  ...
  for(let ind in transactionArray) {
    const txn = transactionArray[ind];
    txnHashesArray.push([txn.hash.toLowerCase()]);
...
TransactionProcessor.prototype.formatTransactionData = function (transactionDataArray, txnIdHash, addressIdHash) {
  ...
  for (let ind in transactionDataArray) {
    ...
    formattedTxn.push(txn.contractAddress ? addressIdHash[txn.contractAddress.toLowerCase()] : null);
    formattedTxn.push(addressIdHash[txn.from.toLowerCase()]);
    formattedTxn.push(addressIdHash[txn.to.toLowerCase()]);

...
TransactionProcessor.prototype.writeTransactionsToDB = async function (transactionsArray) {
  ...
  while (true) {
    ...
    // logger.log("DEBUG", JSON.stringify(addressTypeHash));
    const addressHashIdMap = await oThis.getAddressHashIdMap(addressTypeHash);
 ...
 * @param transactionHashIdMap
 * @param addressHashIdMap
 * @returns {Promise<*>}
 */
...
TransactionProcessor.prototype.formatTransactionData = function (transactionDataArray, txnIdHash, addressIdHash) {
  ...
  const oThis = this
    , txnArray = []
    , invertedTransactionStatus = new TransactionKlass(oThis.chainId).invertedTransactionStatus
    ;
...
TransactionProcessor.prototype.getAddressHashIdMap = async function (addressTypeHash) {
  ...
  if (response.isSuccess()) {
    ...
    logger.error('MemCache return failed for addressType hash');
    return {};
  }
};
...
TransactionProcessor.prototype.populateAddressTypeHash = function (transaction, addressTypeHash) {
  ...
  if (fromAddr.length) {
    addressTypeHash[fromAddr] = addressTypeHash[fromAddr] || {};
    addressTypeHash[fromAddr].address_type = ADDR_TYPE_USER;
  }

  if (toAddr.length) {
    addressTypeHash[toAddr] = addressTypeHash[toAddr] || {};
    addressTypeHash[toAddr].address_type = ADDR_TYPE_USER;
  }

  if (contractAddr.length) {
    addressTypeHash[contractAddr] = addressTypeHash[contractAddr] || {address_type : ADDR_TYPE_CONTRACT};
    if (addressTypeHash[contractAddr].address_type == ADDR_TYPE_USER) {
  ...
  for (let secInd = 0; secInd < transaction.decodedLogs['Transfer'].length; secInd++) {
    ...
    if (contractAddr.length) {
      addressTypeHash[contractAddr] = addressTypeHash[contractAddr] || {address_type : ADDR_TYPE_ERC20};
      if (addressTypeHash[contractAddr].address_type == ADDR_TYPE_USER) {
        addressTypeHash[contractAddr].address_type = ADDR_TYPE_ERC20;
      }
    }
  }
  return addressTypeHash;

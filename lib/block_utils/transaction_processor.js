...
const rootPrefix = "../.."
  ...
  , TransactionLogProcessor = require(rootPrefix + "/lib/block_utils/transaction_processor")
;
  ...
  return oThis.processTransactionsWithIds(transactionArray)
    ...
    .then(async function (i_d_c) {
      if (shouldContinue) {
        ...
        return await new TransactionLogProcessor(oThis.chainId).process(transactionArray);
    ...
    .catch(function (reason) {
      logger.error("transaction processor :: writeTransactionsToDB :: createTxInsertPromise :: writeTokenTransactionToDB rejected the promise.");
      logger.error("reason", reason);
      shouldContinue = false;
      return transactionArray;
    })
...
TransactionProcessor.prototype.processTransactionsWithIds = async function (transactionArray) {
  ...
  for(let ind in transactionArray) {
    ...
    txn.from && addressesHashesArray.push([txn.from]);
    txn.to && addressesHashesArray.push([txn.to]);
    txn.contractAddress && addressesHashesArray.push([txn.contractAddress]);
  }

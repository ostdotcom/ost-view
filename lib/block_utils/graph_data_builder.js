...
GraphDataBuilder.prototype.processTimeFrame = async function (timeFrame) {
  ...
  try {
    ...
    // for failed status, we do not need to do anything as it is handled.
    let lastTimeId;
    let cronData = {};
    cronData[timeFrame] = {};
    ...
    while (true) {
      logger.info("Fetch and Update of graph data from time id ", timeId_1, "to", timeId_2, "for timeFrame", timeFrame);

...
GraphDataBuilder.prototype.perform = async function () {
  ...
  try {
    ...
    return responseHelper.error('l_bu_gdb_1', 'Graph data builder :: perfrom :: try catch' + err);
  }
...
GraphDataBuilder.prototype.fetchAndUpdateGraphData = async function (time_id_1, time_id_2, timeFrame) {
  ...
  try {
    ...
    while(true) {
      ...
      await GraphDataKlass(oThis.chainId).insertMultiple(GraphDataKlass.DATA_SEQUENCE_ARRAY, aggregateDataList)
...
GraphDataBuilder.prototype.processTimeFrame = async function (timeFrame) {
  ...
  let latestTimestamp = oThis.latest_timestamp
  ;

  try {
    //Get last inserted time_id for given timeFrame
    let timeIdResponse = await new GraphDataKlass(oThis.chainId).select('MAX(timed_id) as max_time_id')
      .where({timeFrame: timeFrame})
      .fire();

    let lastTimeId;
    if (timeIdResponse && timeIdResponse[0]['max_time_id']) {
      lastTimeId = timeIdResponse[0]['max_time_id'];
    } else {
      let minTimeIdResponse = await new AggregatedDataModelKlass(oThis.chainId).select('MIN(time_id) as min_time_id')
        .fire();
      if (minTimeIdResponse && timeIdResponse[0]['min_time_id']) {
        lastTimeId = timeIdResponse[0]['min_time_id'];
      } else {
        throw 'No Data found in Aggregate Data table';
      }
    }

    const graphTimeObj = GraphUtils.newInstance(latestTimestamp, timeFrame, lastTimeId);

    graphTimeObj.setGraphStartTime();
    let timeId_1 = graphTimeObj.startTimestamp;
    let timeId_2 = graphTimeObj.getNextTimestamp();
    while (timeId_1 < latestTimestamp) {
      await oThis.fetchAndUpdateGraphData(timeId_1, timeId_2, timeFrame);
      timeId_1 = timeId_2;
      let timeId_2 = graphTimeObj.getNextTimestamp();
    }
  } catch (err) {

  ...
  return Promise.resolve(promiseResult)
    ...
    .then(function (response) {
      ...
      if (response.isInsertSucceeded) {
        ...
        if (response.errInFetchingTxReceipt){
          if(oThis.singleFetchForVerifier){
            logger.win("block_fetcher :: Marking blockNumbers as failed", response.isInsertSucceeded, oThis.blockNumbersArray, response.errInFetchingTxReceipt);
            return oThis.updateBlocksVerified(oThis.blockNumbersArray, blockObj.invertedVerified[blockConst.failed]);
          }else{
            logger.error("block_fetcher :: if blockNumbers are unverified", response.isInsertSucceeded, oThis.blockNumbersArray, response.errInFetchingTxReceipt);
            return false;
          }

        }else{
          logger.win("block_fetcher :: Marking blockNumbers as verified", response.isInsertSucceeded, oThis.blockNumbersArray, response.errInFetchingTxReceipt);
          return oThis.updateBlocksVerified(oThis.blockNumbersArray, blockObj.invertedVerified[blockConst.verified]);
        }

      }else {
        logger.error("block_fetcher :: else blockNumbers are unverified", response.isInsertSucceeded, oThis.blockNumbersArray, response.errInFetchingTxReceipt);
        return response.isInsertSucceeded;
      }
...
BlockFetcher.prototype.updateBlocksVerified = function (blockNumbersArray, verifiedVal) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    blockObj.update({verified: verifiedVal}).where({block_number: blockNumbersArray}).fire().then(
      function (res) {
...
BlockFetcher.prototype.isBlockInserted = async function (blockNumber) {
  ...
  try {
    const result = await new BlockKlass(oThis.chainId).select('COUNT(id) as count_of_id').where({block_number: blockNumber}).fire();
    if (result[0] && result[0]['count_of_id'] === '1' ) {

...
const rootPrefix = "../.."
  ...
  , BlockTokenTransferCacheKlass = require(rootPrefix + '/lib/cache_management/block_token_transfers')
;
...
BlockFetcher.prototype.fetchAndUpdateBlock = async function (blockNumber) {
  ...
  let batchSize = Math.min(MAX_BATCH_SIZE, expectedBatchSize);
  ...
  if (blockNumber === undefined) {
    logger.log("BlockFetcher :: In #fetchBlock undefined blockNumber ");
  ...
  return Promise.all(promiseResult)
    ...
    .then(function (i_d_k) {
      logger.log("BlockFetcher :: fetchAndUpdateBlock :: calling callback");
      return oThis.nextStartBlockNo;
    })
    .catch(function (err) {
      logger.notify('l_bu_bf_1', 'fetchAndUpdateBlock ::error in block fetcher', err);
      return blockNumber;
    });
};
...
BlockFetcher.prototype.filterFetchableBlocks = async function (blockNumber, batchSize) {
  ...
  blockArray = [];
  for (let blockNo=blockNumber; blockNo < blockNumber + batchSize; blockNo++) {
    if (blockHashes[blockNo]) {
      blockArray.push(blockNo);
    }
  }

  logger.log("BlockFetcher :: filterFetchableBlocks :: processed : highestBlockNumber : ", highestBlockNumber, ' blockArray : blockArray.length : ', blockArray.length);
  ...
  return new Promise(function (resolve, reject) {
    ...
    try {
      blockDataArray.forEach(function (blockData) {
        const formattedBlockData = oThis.formatBlockData(blockData);
        //logger.info("Formatted Block data", formattedBlockData);
        blockToInsert.push(blockData.number);
        formattedBlockDataArray.push(formattedBlockData);
      });

      const blockObj = new BlockKlass(oThis.chainId);

      blockObj.insertMultiple(BlockKlass.DATA_SEQUENCE_ARRAY, formattedBlockDataArray).fire()
        .then(function (res) {
          logger.log("Inserting blocks into DB complete");
          oThis.clearBlockCache(blockToInsert);
...
/**
 * Clear cache data for block numbers inserted
 *
 * @param blockNumbers
 */
BlockFetcher.prototype.clearBlockCache = function (blockNumbers){
  const oThis = this;
  for(let i=0;i<blockNumbers.length;i++){
    let blockNumber = blockNumbers[i];
    new BlockCacheKlass({chain_id: oThis.chainId, block_number: blockNumber}).clear();
    new BlockTokenTransferCacheKlass({chain_id: oThis.chainId, block_number: blockNumber}).clear();
  }
};

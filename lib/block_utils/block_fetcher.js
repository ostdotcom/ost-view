...
BlockFetcher.prototype.filterFetchableBlocks = async function (blockNumber, batchSize) {
  ...
  let blockDiff = (highestBlockNumber - DELAY_BLOCK_COUNT) - blockNumber;
  ...
  let response = await new BlockKlass(1409).select('block_number').where({block_number: blockArray}).fire();
...
BlockFetcher.prototype.fetchAndUpdateBlock = async function (blockNumber) {
  ...
  return Promise.resolve(promiseResult)
    .then(function (promiseResult) {
      blockArray = oThis.cleanBlockArray(promiseResult, blockNumber);
      return blockArray;
    })
    .then(function ( i_d_k ) {
      blockArray = oThis.writeBlocksToDB(blockArray);
      return blockArray;
    })
    .then(function ( i_d_k ) {

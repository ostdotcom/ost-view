...
BlockFetcher.prototype.fetchAndUpdateBlock = async function (blockNumber, insertionCompleteCallback) {
  ...
  var highestBlockNumber = await oThis.web3Interact.isNodeConnected()
    .then(function () {
      return oThis.web3Interact.highestBlock();
    })
    .catch(function (err) {
      logger.notify('l_bu_bf_fub_1', 'error in getBlockNumber', err);
      return -1;
    });

  logger.log("fetchAndUpdateBlock :: highestBlockNumber", highestBlockNumber);

  if (highestBlockNumber - (blockNumber + batchSize - 1) < 10) {
    if (oThis.callback) {
  ...
  Promise.resolve(promiseResult)
    ...
    .then(function(isInsertSuccessed){
      ...
      if (isInsertSuccessed){
        logger.win("block_fetcher :: Marking blockNumbers as Varified", isInsertSuccessed, blockNumbersArray);
        return oThis.updateBlocksVerified(blockNumbersArray);
      }else{
        logger.error("block_fetcher :: blockNumbers are NOT Varified", isInsertSuccessed, blockNumbersArray);
 ...
 * @return {Promise}
 */
BlockFetcher.prototype.updateBlocksVerified = function (blockNumbersArray) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    blockObj.update({verified: true}, {touch: false}).where({block_number: blockNumbersArray}).fire().then(
      function (res) {
...
BlockFetcher.prototype.writeBlocksToDB = function (blockDataArray) {
  ...
  logger.log("Inserting blocks into DB start");
...
BlockFetcher.prototype.createReceiptPromise = function ( transaction ) {
  ...
  return oThis.web3Interact.getReceipt(txHash)
    ...
    .catch( function ( reason ) {
      ...
      logger.error("block_fetcher :: writeTransactionsToDB :: createReceiptPromise :: Failed to get tx receipt for ", txHash);

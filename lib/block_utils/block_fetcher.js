...
BlockFetcher.prototype.filterFetchableBlocks = async function (blockNumber, batchSize) {
  ...
  // numbers are always sequential and sorted test and check for hashes .. better not rely
  // TODO: use for (let blockNo = blockNumber; blockNo< blockNumber + batchSize; blockNo++)
  ...
  logger.log("BlockFetcher :: filterFetchableBlocks :: error in get highestBlock, errfetchAndUpdateBlock :: highestBlockNumber and blockArray", highestBlockNumber, blockArray);
...
BlockFetcher.prototype.writeBlocksToDB = function (blockDataArray) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    try {
      ...
      // should never be duplicate on duplicate catch exception

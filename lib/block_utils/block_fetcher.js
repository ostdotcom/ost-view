...
BlockFetcher.prototype.fetchAndUpdateBlock = async function (blockNumber, insertionCompleteCallback) {
  ...
  var highestBlockNumber = await oThis.web3Interact.isNodeConnected()
    .then(function () {
      return oThis.web3Interact.getBlockNumber();
    })
    .catch(function (err) {
  ...
  if (highestBlockNumber - (blockNumber + batchSize - 1) < 2) {
    if (oThis.callback) {
  ...
  Promise.resolve(promiseResult)
    ...
    .catch(function (err) {
        ...
        if (oThis.callback) {
            console.log("BlockFetcher catch", err);
            return oThis.callback(nextStartBlockNo);
        }
    });
};
...
BlockFetcher.prototype.writeTransactionsToDB = async function (blockDataArray) {
  ...
  while(true) {
    const offset = (batchNo - 1) * batchSize
      , batchedData = allTxHashes.slice(offset, offset + batchSize)
      , promiseArray = []
      ;
    ...
    for(var i = 0; i < batchedData.length; i++) {
      promiseArray.push(oThis.web3Interact.getReceipt(batchedData[i]));
    }

    receiptArray = receiptArray.concat(await Promise.all(promiseArray));
  }

  const insertionTransactionArray = oThis.formatTransactionData(transactionsArray, receiptArray, oThis.chainId);

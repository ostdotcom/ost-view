      ...
      if (isInsertSuccessed) {
        ...
        if (errInFetchingTxReceipt){
          if(oThis.singleFetchForVerifier){
            logger.win("block_fetcher :: Marking blockNumbers as Varified", isInsertSuccessed, blockNumbersArray, 2, errInFetchingTxReceipt);
          ...
          logger.win("block_fetcher :: Marking blockNumbers as Varified", isInsertSuccessed, blockNumbersArray, 1, errInFetchingTxReceipt);
...
BlockFetcher.prototype.createReceiptPromise = function (transaction) {
  ...
  return oThis.web3Interact.getReceipt(txHash)
    ...
    .catch(function (reason) {
      ...
      logger.error("block_fetcher :: writeTransactionsToDB :: createReceiptPromise :: Failed to get tx receipt for ", txHash, " errInFetchingTxReceipt :" ,errInFetchingTxReceipt);

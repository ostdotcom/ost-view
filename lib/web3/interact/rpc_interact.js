    ...
    return new Promise(function (resolve) {
      if (contractAddress != undefined && contractAddress.length > 1) {
        ...
        token.methods.balanceOf(address).call()
            ...
            .catch(function (reason) {
              var responseData = responseHelper.error('l_w_i_r_1', 'Something went wrong in contract _getBalance', reason);
        ...
        oThis.web3RpcProvider.eth.getBalance(address)
            ...
            .catch(function (reason) {
              var responseData = responseHelper.error('l_w_i_r_2', 'Something went wrong in ost prime _getBalance', reason);
              return resolve(responseData);
            });
      }
    });

...
web3Interact.prototype = {
  ...
  getReceipt: function (transactionHash) {
    ...
    return this.web3RpcProvider.eth.getTransactionReceipt(transactionHash)
      ...
      .catch( function ( reason ) {
        ...
        oThis.web3Interact.isNodeConnected().then(console.log).catch();
  ...
  getBalance: function (address, contractAddress) {
    ...
    return new Promise(async function (resolve) {
      ...
      const addressDetails = await new AddressIdMapCacheKlass({chain_id:coreConstant.DEFAULT_CHAIN_ID,addresses:[address]}).fetch()
        , addressData = addressDetails.data
      ;

      if (addressData[address].address_type == new AddressesModelKlass(coreConstant.DEFAULT_CHAIN_ID).invertedAddressTypes[addressConst.userAddress]) {
        ...
        var responseData = responseHelper.error('l_w_i_r_3', 'address :' + address + ' is of type contractAddress or erc20');
        return resolve(responseData);
      }
    });

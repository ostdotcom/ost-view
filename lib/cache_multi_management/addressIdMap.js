...
const AddressIdMapCacheKlass = module.exports = function (params) {
  ...
  oThis.addresses = params['addresses'].toLowerCase();
 ...
 * @return {Result}
 */
AddressIdMapCacheKlass.prototype.fetchDataFromSource = async function (cacheAddressHashes) {

  const oThis = this;

  if (!cacheAddressHashes) {
  ...
  const queryResponse = await addressObj.select('id, address_hash, address_type').where(["address_hash IN (?)", cacheAddressHashes]).fire();

  if (queryResponse.length === 0) {
    return responseHelper.error(
      'cmm_adIdMap_2', 'AddressIdMapCacheKlass : Data not found for : ' + cacheAddressHashes
    );
  }

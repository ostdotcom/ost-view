...
AddressIdMapCacheKlass.prototype.fetchDataFromSource = async function (cacheAddressHahses) {
  ...
  const queryResponse = await addressObj.select('*').where(["address_hash IN (?)", cacheAddressHahses]).fire();

  if (queryResponse.length === 0) {
    return responseHelper.error(
      'cmm_adIdMap_2', 'AddressIdMapCacheKlass : Data not found for : '+cacheAddressHahses
    );
  }

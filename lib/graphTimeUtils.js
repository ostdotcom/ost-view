  ...
  oThis.methodMap = {
    'Week':{
        ...
        getStartTime: function(currentTimestamp){
          var timeStamp = moment(currentTimestamp*1000 + this.quantum*1000).startOf('day').unix();
          return (timeStamp - 7*this.quantum);
        },
        getNextTime: function(currentTimestamp){
          return moment(currentTimestamp*1000 + this.quantum*1000).startOf('day').unix();
        }
      },
    'Month':{
      quantum: 24*60*60,
    ...
    'Day':{
      quantum: 60*60,
      getStartTime: function(currentTimestamp){
        var timeStamp = moment(currentTimestamp*1000 + this.quantum*1000).startOf('hour').unix();
        return (timeStamp - 24*this.quantum);
      },
      getNextTime: function(currentTimestamp){
        return moment(currentTimestamp*1000 + this.quantum*1000).startOf('hour').unix();
      }
    },
    'Year':{
      getStartTime: function(currentTimestamp){
        var currdate = new Date(currentTimestamp*1000)
          , lastYearNextMonthStart = moment(new Date(currdate.getFullYear()-1, currdate.getMonth()+1, 1).getTime()).startOf('month').unix();
      ...
      getNextTime: function(currentTimestamp){
        ...
        return moment(timeinMilli + days*dayMilliSeconds).startOf('month').unix();
      }
    },
    'All':{
      getStartTime: function(currentTimestamp){
         var monthStartTime = moment(currentTimestamp*1000).startOf('month').unix();

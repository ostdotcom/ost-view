...
const GraphTimeUtilsKlass = function(latestBlockTimestamp, timeFrame, blockStartTimestamp){
  ...
  oThis.methodMap = {
    'week':{
        ...
        getStartTimeFrame: function(currentTimestamp) {
    ...
    'month':{
      ...
      getStartTimeFrame: function(currentTimestamp) {
        ...
        return moment(new Date(currDate)).utc().startOf('month').unix();
      }
    },
    'hour':{
      quantum: 5*60,
      getStartTime: function(currentTimestamp){
        return (currentTimestamp - (currentTimestamp % this.quantum)) - 12*this.quantum  ;
      },
      getNextTime: function(currentTimestamp){
    ...
    'year':{
      ...
      getStartTimeFrame: function(currentTimestamp) {
        return moment(currentTimestamp*1000 + this.quantum*1000).utc().startOf('year').unix();
      }
    },
    'all':{
      getStartTime: function(currentTimestamp){
...
GraphTimeUtilsKlass.prototype = {
  ...
  getRoundOffTimestamp: function(timestamp) {
    const oThis = this;
    return oThis.methodMap[oThis.timeFrame].getStartTimeFrame(timestamp);
  },

...
const HomePageCacheKlassPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    const brandedTokenModelObject =  new BrandedTokenModelKlass(oThis.chainId)
      ...
      , marketCapData = await brandedTokenStatsModelObject.select('SUM(market_cap) AS marketCap').fire()
      , marketCap = marketCapData[0].marketCap
      , tokenTransfersData = await brandedTokenStatsModelObject.select('SUM(token_transfers) AS tokenTransfers').fire()
      , tokenTransfers = tokenTransfersData[0].tokenTransfers
      , tokenOstVolumeData = await brandedTokenStatsModelObject.select('SUM(token_ost_volume) AS tokenOstVolume').fire()
      , tokenOstVolume = tokenOstVolumeData[0].tokenOstVolume
    ...
    console.log("*************HomePageCacheKlass -> finalFormattedData : ",finalFormattedData);

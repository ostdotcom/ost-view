...
const TopUsersCacheKlassPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    if (addressIdDetails.isFailure() || !addressIdData[oThis.contractAddress]){
      return responseHelper.error('cm_tu_1', 'Data not available for : '+oThis.contractAddress);
    }

    const addressId = addressIdData[oThis.contractAddress].id
    ;

    const topUsers = await addressDetailsObject.select('id, address_id,  contract_address_id, tokens, tokens_earned, tokens_spent, total_transactions, total_token_transfers')

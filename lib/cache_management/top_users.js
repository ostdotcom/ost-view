  ...
  , getFormattedTopUsersData : function (topUsers, addresses) {
    ...
    for (let i=0; i<topUsers.length; i++){
     ...
     ;
      ...
      user['tokens'] = TokenUnits.convertToNormal(user['tokens']).toFormat(5);
      user['tokens_earned'] = TokenUnits.convertToNormal(user['tokens_earned']).toFormat(5);
      user['tokens_spent'] = TokenUnits.convertToNormal(user['tokens_spent']).toFormat(5);

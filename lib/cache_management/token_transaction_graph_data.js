 ...
 *
 */
const TokenTransactionGraphData = function(params) {
...
TokenTransactionGraphData.prototype = Object.create(baseCache.prototype);
...
const TokenTransferGraphDataPrototype = {
  ...
  setCacheKey: function() {
    ...
    oThis.cacheKey = oThis._cacheKeyPrefix() + "ttrgd_" + 'cid_' + oThis.contractAddressId + 'd_' + oThis.duration ;
  ...
  fetchDataFromSource: async function() {
    ...
    if (String(oThis.duration) !== 'hour'){
      ...
      graphDataObjectResponse = await graphDataObject.select('timestamp, SUM(total_transactions) as token_transactions, SUM(token_ost_volume) as token_ost_volume')
      ...
      graphDataObjectResponse = await graphDataObject.select('timestamp, SUM(total_transactions) as token_transactions, SUM(token_ost_volume) as token_ost_volume').where(['contract_address_id'+ condition + '?' , oThis.contractAddressId])
        .group_by('timestamp')
        .order_by('timestamp desc')
        .limit(30)
        .fire();
    ...
    if (graphDataObjectResponse) {
      ...
      logger.error('TokenTransactionGraphData :: fetchDataFromSource :: graphDataObjectResponse is null');
...
Object.assign(TokenTransactionGraphData.prototype, TokenTransferGraphDataPrototype);

module.exports = TokenTransactionGraphData;

/*
 TokenTransaction = require('./lib/cache_management/token_transaction_graph_data')
 new TokenTransaction({chain_id : 1409, contract_address_id: 35, duration: 'hour'}).fetchDataFromSource().then(console.log);
*/
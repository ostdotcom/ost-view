...
const TokenTransferGraphDataPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    if (String(oThis.duration) !== 'hour'){
      ...
      graphDataObjectResponse = await graphDataObject.select('timestamp, SUM(total_transfers) as token_transfers, SUM(token_ost_volume) as token_ost_volume').where(['contract_address_id'+ condition + '?' , oThis.contractAddressId])
        .group_by('timestamp')
        .order_by('timestamp desc')
        .limit(30)
        .fire();

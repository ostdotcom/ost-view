...
const TokenTransferGraphDataPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    graphDataObjectResponse = await graphDataObject.select('contract_address_id, branded_token_transaction_type_id, SUM(total_transfers) as token_transfers, SUM(token_ost_volume) as token_ost_volume')
  ...
  processResponse: async function (graphDataObjectResponse) {
    ...
    if (graphDataObjectResponse instanceof Array && graphDataObjectResponse.length > 0) {
      ...
      const cacheResponse = new BTTransactionTypesCacheKlass({chain_id: oThis.chainId, ids: typeIds}).fetch();

...
const TokenTransferGraphDataPrototype = {
  ...
  fetchDataFromSource: async function() {
    ...
    if (String(oThis.duration) !== 'hour' && String(oThis.duration) !== 'day'){
      ...
      graphDataObjectResponse = await graphDataObject.select('contract_address_id, branded_token_transaction_type_id, SUM(total_transactions) as token_transactions')
        .where(['contract_address_id=? AND time_frame=? AND timestamp>=? AND branded_token_transaction_type_id!=0', oThis.contractAddressId, graphDataObject.invertedTimeFrames[oThis.getTimeFrame()], limitTime])
      ...
      graphDataObjectResponse = await graphDataObject.select('contract_address_id, branded_token_transaction_type_id, SUM(total_transactions) as token_transactions')
        .where(['contract_address_id=? AND timestamp>=? AND branded_token_transaction_type_id!=0', oThis.contractAddressId, limitTime])
        .group_by('contract_address_id, branded_token_transaction_type_id')
        .limit(200)
        .fire();
    }

...
MySQL.prototype = {
   ...
   */
  selectLastInsertedTimeStamp: function () {
    var oThis = this;
   ...
   * @param timestamp1 - timestamp left endpoint
   * @param timestamp2 - timestamp right endpoint
   * @returns {Promise}
   */
  ...
  selectAggregateDataWithinTimestamp: function(contractId, timeId1, timeId2, groupByTransactionType) {
    ...
    var query = "SELECT SUM(total_transactions), SUM(total_transaction_value), SUM(total_transfers), SUM(total_transfer_value) from aggregate WHERE time_id>=? AND time_id<? AND company_token_id=?";
  ...
  insertOrUpdateCompanyData: function(dataArray) {
    var oThis = this;
    logger.log("Insert into table with data", constants.COMPANY_TOKEN_TABLE_NAME, dataArray);

    var query = "INSERT INTO " + constants.COMPANY_TOKEN_TABLE_NAME + " " + constants.COMPANY_TOKEN_DATA_SEQUENCE +
        " VALUES ? ON DUPLICATE KEY UPDATE company_token.company_name=VALUES(company_token.company_name), company_token.company_symbol=VALUES(company_token.company_symbol), " +
        "company_token.price=VALUES(company_token.price), company_token.token_holders=VALUES(company_token.token_holders), " +
        "company_token.market_cap=VALUES(company_token.market_cap), company_token.circulation=VALUES(company_token.circulation), " +
        "company_token.total_supply=VALUES(company_token.total_supply), company_token.transactions_data=VALUES(company_token.transactions_data), " +
        "company_token.transactions_volume_data=VALUES(company_token.transactions_volume_data), company_token.tokens_transfer_data=VALUES(company_token.tokens_transfer_data), " +
        "company_token.tokens_volume_data=VALUES(company_token.tokens_volume_data)";
      ...
      try {
        oThis.con.query(query, [dataArray], function (err, result) {
          if (err){
            ...
            logger.info("Insertion in " + constants.COMPANY_TOKEN_TABLE_NAME + " successful");
            resolve(result);
          }
        });
      } catch (err) {
   ...
   * @returns {Promise}
   */
  selectTotalTokenHolders: function(companyTokenId) {
    var oThis = this;
    logger.log("Getting total tokens holders of company token id ", companyTokenId);
    var query = "SELECT COUNT(*) from " + constants.ADDRESS_TABLE_NAME + " WHERE company_token_id=?";
    return new Promise(function (resolve, reject) {
      try {

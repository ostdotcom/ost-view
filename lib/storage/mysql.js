...
MySQL.prototype = {
  ...
  selectAggregateDataWithinTimestamp: function (contractId, timeId1, timeId2, groupByTransactionType) {
    ...
    if (groupByTransactionType) {
      ...
      "WHERE time_id>=? AND time_id<? AND transaction_type_id!=0 AND branded_token_id" + subQuery;
...
// market_cap <= 45000000000000000000000 ORDER BY market_cap DESC , id asc limit 0, 10
  ...
  selectTokenStatsData: function (brandedTokenId) {
    ...
    //Total_transfers changed to total transactions because of airdrop aggregate of 3 transfers make one transaction
    logger.info("#selectTokenStatsData :: Branded token id", brandedTokenId);
    var query = "select SUM(total_transactions), SUM(total_transfer_value) from  " + constants.AGGREGATE_TABLE_NAME + " where branded_token_id" + subQuery;
    return new Promise(function (resolve, reject) {
      logger.log(query);
      try {
        oThis.con.query(query, brandedTokenId, function (err, result) {
          if (err) {
            ...
            resolve({token_transfers:result[0]['SUM(total_transactions)'], token_volume: result[0]['SUM(total_transfer_value)']});
          }
        });
      } catch (err) {
        logger.error(err);

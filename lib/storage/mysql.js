...
MySQL.prototype = {
  ...
  selectAggregateDataWithinTimestamp: function(contractId, timeId1, timeId2, groupByTransactionType) {
    ...
    if (groupByTransactionType) {
      query = "SELECT b.transaction_type, a.transaction_type_id, SUM(a.total_transactions), SUM(a.total_transaction_value), " +
          ...
          "WHERE a.time_id>=? AND a.time_id<? AND a.branded_token_id=? AND b.id = a.transaction_type_id";
    } else {
    ...
    if (groupByTransactionType) {
      query = query + " GROUP BY transaction_type_id";
    }
    return new Promise(function (resolve, reject) {
      try {
      ...
      try {
        oThis.con.query(query, contractAddress ,function (err, result, fields) {
          if (err){
            ...
            logger.info(JSON.parse(result[0]['tokens_transfer_data'].toString()));
  ...
  /**
   * Get Graph of Number of Branded Token Transactions
   * @param contractAddress
   * @returns {Promise}
   */
  getGraphDataOfNumberOfBrandedTokenTransactions: function(contractAddress){
    var oThis = this;
    logger.log("Getting graph of branded token contract address ", contractAddress);
    var query = "SELECT tokens_volume_data FROM "+ constants.BRANDED_TOKEN_TABLE_NAME + " WHERE contract_address=?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, contractAddress ,function (err, result, fields) {
          if (err){
            ...
            logger.info(JSON.parse(result[0]['tokens_volume_data'].toString()));
   ...
   * @return {Promise}
   */
  getGraphDataForBrandedTokenTransactionsByType: function(contractAddress){
    var oThis = this;
    logger.log("Getting graph of branded token contract address ", contractAddress);
    var query = "SELECT transaction_type_data FROM "+ constants.BRANDED_TOKEN_TABLE_NAME + " WHERE contract_address=?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, contractAddress ,function (err, result, fields) {
          if (err){
            ...
            logger.info(JSON.parse(result[0]['transaction_type_data'].toString()));
   ...
   * @return {Promise}
   */
  getBrandedTokenTopUsers: function(brandedTokenId){
    var oThis = this;

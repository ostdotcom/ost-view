...
MySQL.prototype = {
  ...
  selectAddressesWithBrandedToken: function (brandedTokenId){
    var oThis = this;
    logger.log("select addresses with branded token",brandedTokenId);

    var query = "select address, tokens from  " + constants.ADDRESS_TABLE_NAME + ' where branded_token_id=?';
    return new Promise(function (resolve, reject) {
  ...
  selectRecentTokenTransactions: function(pageNumber, pageSize){
    var oThis = this;
    logger.log("Getting  Recent Token Transactions FROM TOKEN TRANSACTION" );
    var query = "SELECT * from " + constants.TOKEN_TRANSACTION_TABLE_NAME + " ORDER BY timestamp DESC LIMIT ?,?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, [((pageNumber - 1) * pageSize), pageSize] ,function (err, result, fields) {
          if (err){
            logger.error(err);
            reject(err)
          } else{
  ...
  selectTopTokens:  function(pageNumber, pageSize){
    var oThis = this;
    logger.log("Getting  Recent Token Transactions FROM TOKEN TRANSACTION" );
    var query = "SELECT id, company_name, contract_address, company_symbol, price, market_cap, circulation from " + constants.BRANDED_TOKEN_TABLE_NAME + " ORDER BY market_cap DESC LIMIT ?,?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, [((pageNumber - 1) * pageSize), pageSize] ,function (err, result, fields) {
          if (err){

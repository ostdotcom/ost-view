...
MySQL.prototype = {
  ...
  selectCoinFromContractAddress: function (contractAddress) {
    ...
    var query = "SELECT * from " + constants.BRANDED_TOKEN_TABLE_NAME + " WHERE contract_address=?";

    return new Promise(function (resolve, reject) {
      try {
        var pre_query = Date.now();
        var qry = oThis.con.query(
          query,
          contractAddress,
          function (err, result, fields) {
  ...
  selectBrandedTokenFromId: function (brandedTokenId) {
    ...
    var query = "SELECT company_name,contract_address,company_symbol, symbol_icon, price,token_holders,market_cap,circulation,total_supply,uuid, token_transfers, token_ost_volume, creation_time from " + constants.BRANDED_TOKEN_TABLE_NAME + " WHERE id=?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query,
          brandedTokenId,
          function (err, result, fields) {
            if (err) {
              ...
              logger.log('mysql#selectBrandedTokenFromId result count --> ', result.length);
              if (result.length > 0){
                resolve(result[0]);
              }else{
                resolve();
   ...
   * @return {Promise}
   */
  selectBlockTokenTransactionCountBlockNumber: function (blockNumber) {
    var oThis = this;

    var query = "select COUNT(*) from " + constants.TOKEN_TRANSACTIONS_TABLE_NAME + " WHERE block_number=?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query,
          blockNumber,
          function (err, result, fields) {
            if (err) {
              ...
              //logger.info("no of results found -> ", result.length,blockNumber);
              //logger.info(result);
   ...
   * @return {Promise}
   */
  selectHighestInsertedBlock: function (min_block, max_block) {
    var oThis = this;
    logger.log("Getting higest block Number");
    var query = "SELECT MAX(block_number) from " + constants.BLOCK_TABLE_NAME;
    var whereClause = '';
    if (min_block && max_block){
      whereClause = ' where block_number >= ' + min_block + ' and block_number < ' + max_block ;
      query += whereClause;
    }
  ...
  selectLowestUnVerifiedBlockNumber: function() {
    ...
    const query = 'SELECT MIN(block_number) as minBlock from ' + constants.BLOCK_TABLE_NAME + " WHERE verified=0";
  ...
  selectContractLedger: function (contractAddress, pageSize, pagePayload) {
    ...
    logger.log("selectContractLedger :: ",query,whereValues);
    return new Promise(function (resolve, reject) {
      try {
  ...
  /**
   * To get List of token transactions of the block based on its provided block number
   * @param  {Integer} blockNumber Block Number
   * @param  {Integer} pagePayload page payload
   * @param  {Integer} pageSize Page Size
   * @return {Promise}
   */
  selectBlockTokenTransactionsFromBlockNumber: function (blockNumber, pageSize, pagePayload) {
    var oThis = this;
    var query = "SELECT * from " + constants.TOKEN_TRANSACTIONS_TABLE_NAME + " WHERE block_number = ?";
    ...
    if(!pagePayload){
      ...
      if(!isNaN(parseInt(pagePayload.timestamp)) && !isNaN(parseInt(pagePayload.id))){
        ...
        if(pagePayload.direction === "prev") {
          doReverse = true;
          query += " and timestamp >= ? and id >= ? order by timestamp asc,id asc"
        }
        else if(pagePayload.direction === "next"){
          query += " and timestamp <= ? and id <= ? order by timestamp desc,id desc"
        }
        query += " limit ?";
        whereValues = [blockNumber, timestamp, transactionId, pageSize]
      } else {
        return Promise.reject("selectBlockTokenTransactionsFromBlockNumber Invalid payload");
      }
    }
  ...
  selectAddressDetailsTotalTransactionCount: function(address){
    var oThis = this;
    var query = "SELECT COUNT(*) from " + constants.ADDRESS_TOKEN_TRANSACTION_TABLE_NAME + " WHERE address=?";
  ...
  /**
   * To insert Block Data into the provided table based on column sequence
   * @param  {String} tableName Table Name
   * @param  {String} columnsSequence Column Sequence
   * @param  {Array} dataArray Data
   * @return {Promise}
   */
  insertBlockDataArray: function (tableName, columnsSequence, dataArray) {
    var oThis = this;
    ...
    query += 'ON DUPLICATE KEY UPDATE block_number=block_number';
    ...
    while (true) {
      ...
      var offset = ((batchNo - 1) * batchSize);

      if (offset > dataArray.length) break;

      var queryData = dataArray.slice(offset, offset + batchSize);

      if (queryData.length <= 0) break;

      logger.log('inserting batch no - ', batchNo);
      batchNo++;

      promiseArray.push(
       new Promise(function (onResolve, onReject) {
         logger.info('inserting block data - ', queryData.length);

         try {
            var qry = oThis.con.query(query, [queryData], function (err, result) {
              if (err) {
                logger.error(err);
                onResolve();
              } else {
                logger.info("Insertion in block table successful");
                onResolve(result);
              }
            });
          } catch (err) {
   ...
   * @return {Promise}
   */
  insertDataForBlockFetcher: function (tableName, columnsSequence, data) {
    var oThis = this;
    ...
    while(true){
      ...
      var queryData = data.slice(offset, offset+batchSize);

      if(queryData.length <=0 ) break;

      logger.info('inserting batch no - ', batchNo);
      batchNo++;

      promiseArray.push(
      new Promise(function (onResolve, onReject) {
        logger.info('inserting data for block fetcher - tableName :' ,tableName ," queryData.length:", queryData.length);
   ...
   * @return {Promise}
   */
  insertDataForTransactionHashTables: function (tableName, columnsSequence, data) {
    const oThis = this;
    ...
    if (data[0].constructor === Array) {
      ...
      logger.log('UNREACHABLE CODE triggered.');
      query += (" " + "VALUES (?)");
    }

    query += ' ON DUPLICATE KEY UPDATE transaction_hash=transaction_hash';
    ...
    while(true){
      ...
      promiseArray.push(
        new Promise(function (onResolve, onReject) {
          logger.info("Inserting data for transaction hash tables : ",queryData.length);
   ...
   * @param  {String} columnsSequence Column Sequence
   * @param  {Array} data Data
   * @param  {String} onDuplicateStatement onDuplicate key update statement (optional)
   * @return {Promise}
   */
  bulkInsert: function (tableName, columnsSequence, dataArray, onDuplicateStatement) {
    var oThis = this;
    //logger.debug("Insert into table", tableName, dataArray);
    if (dataArray == undefined || dataArray.length < 1) {
      return Promise.resolve("No data");
    }
    var query = "INSERT INTO " + tableName + " " + columnsSequence + " VALUES ?";

    if (onDuplicateStatement){
      query += " ON DUPLICATE KEY UPDATE " + onDuplicateStatement;
    }
    ...
    while(true){
      ...
      promiseArray.push(
        new Promise(function (onResolve, onReject) {
          logger.info("Inserting bulk insert in ",tableName, queryData.length);
          try {
            var qry = oThis.con.query(query, [queryData], function (err, result) {
              if (err) {
                logger.error(err);
   ...
   * @return {Promise}
   */
  insertDataForIdTables: function (tableName, columnsSequence, data) {
    const oThis = this;
    ...
    if (data[0].constructor === Array) {
      ...
      logger.info('UNREACHABLE CODE triggered.');
      query += (" " + "VALUES (?)");
    }
    ...
    while(true){
      ...
      promiseArray.push(
        new Promise(function (onResolve, onReject) {
          logger.log("Inserting data into tableName:",tableName," queryData: ",queryData.length);
          try {
            var qry = oThis.con.query(query, [queryData], function (err, result) {
              if (err) {
                logger.error(err);
  ...
  insertOrUpdateAddressData: function (dataArray) {
    ...
    var query = "INSERT INTO " + constants.ADDRESS_TABLE_NAME + " " + constants.ADDRESS_DATA_SEQUENCE +
      " VALUES ? ON DUPLICATE KEY UPDATE address.tokens=VALUES(address.tokens), " +
  ...
  selectAggregateDataWithinTimestamp: function (contractId, timeId1, timeId2, groupByTransactionType) {
    ...
    if (groupByTransactionType) {
      ...
      query = "SELECT SUM(total_transactions), SUM(total_transaction_value), SUM(total_transfers), SUM(total_transfer_value) from aggregate " +
      "WHERE time_id>=? AND time_id<? AND branded_token_id" + subQuery + " AND transaction_type_id!=0";
      //transaction_type_id is kept not equal to 0 so that no transaction with transaction type will come in data.
    }

    return new Promise(function (resolve, reject) {
      logger.info("inserting into aggregate dataWithin timestamp : timeId1:", timeId1, "timeId2:", timeId2," contractId:",contractId);

      try {
        var qry = oThis.con.query(query, [timeId1, timeId2, contractId], function (err, result, fields) {
          if (err) {
            logger.error(err);
            reject(err)
          } else {
            logger.info("no of results found in selectTransferAggregateDataWithinTimestamp -> ", result.length);
            //logger.debug(result);
            resolve(result);
          }
        });
  ...
  /**
   * Get Transfer Aggregate data within time stamp
   * @param {Integer} contractId - Contract Id of the company
   * @param {Integer} timeId1 - left bound of timestamp
   * @param {Integer} timeId2 - right bound of timestamp
   * @param {Boolean} groupByTransactionType - should do group by on transaction type
   */
  selectTransferAggregateDataWithinTimestamp: function (contractId, timeId1, timeId2) {
    var oThis = this;
    logger.log("Getting transaction within timestamps", timeId1, timeId2);
    var query;
    ...
    query = "SELECT SUM(total_transactions), SUM(total_transaction_value), SUM(total_transfers), SUM(total_transfer_value) from aggregate " +
    "WHERE time_id>=? AND time_id<? AND branded_token_id" + subQuery;
  ...
  insertOrUpdateCompanyData: function (dataArray) {
    ...
    var query = "INSERT INTO " + constants.BRANDED_TOKEN_TABLE_NAME + " " + constants.BRANDED_TOKEN_DATA_SEQUENCE +
      ...
      "branded_token.creation_time=VALUES(branded_token.creation_time), branded_token.symbol_icon=VALUES(branded_token.symbol_icon)";
    ...
    while(true){
      ...
      promiseArray.push(
        new Promise(function (onResolve, onReject) {
          logger.info("Inserting into ",constants.BRANDED_TOKEN_TABLE_NAME,"dataArray : ",queryData.length)
          try {
            var qry = oThis.con.query(query, [queryData], function (err, result) {
              if (err) {
                logger.error(err);
  ...
  /**
   * Get transfers and volumes of Branded Token Transactions
   * @param brandedTokenId Branded Token Id
   * @returns {Promise}
   */
  selectTransfersAndVolumesOfBrandedTokenTransactions: function (brandedTokenId) {
    var oThis = this;
    logger.log("Getting graph of branded token contract address ", brandedTokenId);
      ...
      try {
        oThis.con.query(query, brandedTokenId, function (err, result, fields) {
          if (err) {
            ...
            if(result && result.length>0 && result[0]['tokens_volume_data']){
      ...
      try {
        oThis.con.query(query, brandedTokenId, function (err, result, fields) {
          if (err) {
            ...
            if(result && result.length>0 && result[0]['transaction_type_data']){
      ...
      try {
        var qry = oThis.con.query(query, [brandedTokenId, parseInt(topUsersCount)], function (err, result, fields) {
          if (err) {
            ...
            logger.info("mysql#selectBrandedTokenTopUsers result count--> ", qry.sql);
            resolve(result);
          }
        });
      } catch (err) {
  ...
  selectBrandedTokenDetails: function () {
    ...
    var query = "SELECT id, company_name, contract_address, company_symbol, uuid, price, token_holders, market_cap, circulation, total_supply, token_transfers, token_ost_volume, creation_time, symbol_icon from " + constants.BRANDED_TOKEN_TABLE_NAME;
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, function (err, result, fields) {
          if (err) {
  ...
  insertIntoTransactionTypeId: function (contractAddress, tag) {
    ...
    return new Promise(function (resolve, reject) {
      logger.info("Inserting into TRANSACTION_TYPE_ID : tag:",tag," for contractAddress:",contractAddress);
      try {
  ...
  insertIntoTransactionType: function (transactionHash, tagId) {
    ...
    return new Promise(function (resolve, reject) {
      logger.info("Inserting into TRANSACTION_TYPE : tagId:",tagId," for transactionHash:",transactionHash);
      try {
  ...
  selectRecentTokenTransactions: function (pageSize, pagePayload) {
    ...
    if(!pagePayload){
      ...
      if(!isNaN(parseInt(pagePayload.timestamp)) && !isNaN(parseInt(pagePayload.id))){
        ...
        if(pagePayload.direction === "prev") {
          doReverse = true;
          query += " where timestamp >= ? and id >= ? order by timestamp asc, id asc"
        }
        else if(pagePayload.direction === "next"){
          query += " where timestamp <= ? and id <= ? order by timestamp desc, id desc"
        }
        query += " limit ?";
  ...
  selectTopTokens: function (pageSize, pagePayload) {
    ...
    var query = "SELECT id, company_name, contract_address, company_symbol, price, market_cap, circulation, token_ost_volume, symbol_icon from " + constants.BRANDED_TOKEN_TABLE_NAME + " where id != ? ";

//SELECT id, company_name, contract_address, company_symbol, price, market_cap, circulation from branded_token where
// market_cap <= 45000000000000000000000 ORDER BY market_cap DESC , id asc limit 0, 10

    var whereValues = [],
      timestamp = pagePayload.timestamp,
      pageNumber = pagePayload.page_no
      ;

    query += " and creation_time <= ? order by market_cap desc, id asc limit ?, ?";
    whereValues = [constants.BASE_CONTRACT_ADDRESS, timestamp, (pageNumber-1)*(pageSize-1),pageSize];
  ...
  /**
   * To get Transfer Token aggregate details
   * @param brandedTokenId - Branded token Id
   * @returns {*|wrappedPromise|null}
   */
  selectTransferTokenStatsData: function (brandedTokenId) {
    var oThis = this;
    var subQuery;
    if (brandedTokenId == 0) {
      subQuery = '!=?';
    ...
    logger.info("#selectTokenStatsData :: Branded token id", brandedTokenId);
    var query = "select SUM(total_transactions), SUM(total_transfers) from  " + constants.AGGREGATE_TABLE_NAME + " where branded_token_id" + subQuery + " AND transaction_type_id!=0";
    return new Promise(function (resolve, reject) {
      logger.log(query);
      try {
        oThis.con.query(query, brandedTokenId, function (err, result) {
          if (err) {
            ...
            resolve({token_transactions:result[0]['SUM(total_transactions)'], token_transfers: result[0]['SUM(total_transfers)']});
          }
        });
      } catch (err) {
        logger.error(err);
  ...
  ,selectContractAddressFromBrandedTokenNameOrSymbol: function (brandedTokenNameOrSymbol){
    var oThis = this;

    logger.info("#selectContractAddressFromBrandedTokenNameOrSymbol :: Branded token name or symbol", brandedTokenNameOrSymbol);
    var query = "select contract_address from "+constants.BRANDED_TOKEN_TABLE_NAME+" where company_name=? or company_symbol=?";
    return new Promise(function (resolve, reject) {
      logger.log(query);
      try {
        oThis.con.query(query, [brandedTokenNameOrSymbol, brandedTokenNameOrSymbol], function (err, result) {
          if (err) {
            logger.error(err);
            reject(err)
          } else {
            logger.log("mysql#selectContractAddressFromBrandedTokenNameOrSymbol Branded token name or symbol -->", result.length);
            if (result.length > 0){
              resolve(result[0]['contract_address']);
            }else{
              reject(err)
            }
          }

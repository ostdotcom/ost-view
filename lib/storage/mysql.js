...
MySQL.prototype = {
  ...
  selectLowestUnVerifiedBlockNumber: function() {
    ...
    const query = 'SELECT MIN(block_number) as minBlock from ' + constants.BLOCK_TABLE_NAME + " WHERE verified=false";
  ...
  insertBlockDataArray: function (tableName, columnsSequence, dataArray) {
    ...
    while (true) {
      ...
      console.log('inserting batch no - ', batchNo);
      batchNo++;
  ...
  insertDataForTransactionHashTables: function (tableName, columnsSequence, data) {
    ...
    if (data[0].constructor === Array) {
      ...
      console.log('UNREACHABLE CODE triggered.');
      query += (" " + "VALUES (?)");
    }
    ...
    return new Promise(function (resolve, reject) {
      try {
        var qry = oThis.con.query(query, [brandedTokenId, parseInt(topUsersCount)], function (err, result, fields) {
          //console.log("mysql#selectBrandedTokenTopUsers --> ", qry.sql);
          if (err) {
            logger.error(err);
            reject(err)
          } else {
            console.log("mysql#selectBrandedTokenTopUsers result count--> ", qry.sql);
            resolve(result);
          }
        });
      } catch (err) {

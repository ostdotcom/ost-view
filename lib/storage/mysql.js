...
MySQL.prototype = {
  ...
  selectTransaction: function (transactionHash) {
    ...
    var query = "SELECT * from " + constants.TRANSACTION_TABLE_NAME + " WHERE transaction_hash=?";
  ...
  selectCoinFromContractAddress: function (contractAddress) {
    ...
    var query = "SELECT company_name,contract_address,company_symbol,price,token_holders,market_cap,circulation,total_supply,uuid from " + constants.BRANDED_TOKEN_TABLE_NAME + " WHERE contract_address=?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query,
          contractAddress,
  ...
  selectBrandedTokenFromId: function (brandedTokenId) {
    ...
    var query = "SELECT company_name,contract_address,company_symbol,price,token_holders,market_cap,circulation,total_supply,uuid from " + constants.BRANDED_TOKEN_TABLE_NAME + " WHERE id=?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query,
          brandedTokenId,
  ...
  selectContractLedger: function (contractAddress, pageNumber, pageSize) {
    ...
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query,
          [contractAddress, (pageNumber * pageSize), pageSize],
          function (err, result, fields) {
            if (err) {
              logger.error(err);
              reject(err);
      ...
      try {
        oThis.con.query(query,'0', function (err, result) {
          if (err){
            ...
            logger.info("select in " + constants.BRANDED_TOKEN_TABLE_NAME + " successful");
  ...
  selectBrandedTokenIdFromContract: function (contractAddress){
    var oThis = this;
    logger.log("select chain data");

    var query = "select id from  " + constants.BRANDED_TOKEN_TABLE_NAME + ' where contract_address=?';
    return new Promise(function (resolve, reject) {

      logger.log(query);
      try {
        oThis.con.query(query,contractAddress, function (err, result) {
          if (err){
            logger.error(err);
            reject(err)
          } else {
            if (result.length > 0) {
              ...
              logger.info("select in " + constants.ADDRESS_TABLE_NAME + " successful");
               resolve(result[0]['id']);
            }else{
              resolve()
            }
  ...
  selectAddressesWithBrandedToken: function (brandedTokenId, pageNumber, pageSize){
    var oThis = this;
    logger.log("select addresses with branded token",brandedTokenId,(pageNumber*pageSize),pageSize);

    var query = "select address, tokens from  " + constants.ADDRESS_TABLE_NAME + ' where branded_token_id=? LIMIT ?,?';
    return new Promise(function (resolve, reject) {

      logger.log(query);
      try {
        oThis.con.query(query,[brandedTokenId,(pageNumber*pageSize),pageSize], function (err, result) {
          if (err){
            logger.error(err);
            reject(err)
          } else{
            logger.info("select in " + constants.ADDRESS_TABLE_NAME + " successful", result);
            resolve(result);
          }
        });
      } catch (err) {
  ...
  /**
   * Get Graph data for branded token transactions by type
   * @param  {String} contractAddress - contract address
   *
   * @return {Promise}
   */
  selectGraphDataForBrandedTokenTransactions: function(contractAddress){
    var oThis = this;
    logger.log("Getting graph of  branded token transactions contract address ", contractAddress);
    var query = "SELECT transactions_volume_data FROM "+ constants.BRANDED_TOKEN_TABLE_NAME + " WHERE contract_address=?";
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, contractAddress ,function (err, result, fields) {
          if (err){
            ...
            if (result !== undefined && result.length > 0){
              logger.info(JSON.parse(result[0]['transactions_volume_data'].toString()));
  ...
  selectRecentTokenTransactions: function(pageNumber, pageSize, pagePayload){
    var oThis = this;
    logger.log("Getting  Recent Token Transactions FROM TOKEN TRANSACTION" ,pageNumber);
    var query = "SELECT * from " + constants.TOKEN_TRANSACTION_TABLE_NAME + " where ";
    //SELECT * from token_transactions where timestamp <= 1518696534 ORDER BY timestamp DESC LIMIT 5

    var timestamp
      ,orderBy
      ;
    if (pagePayload.timestamp !== undefined){
      timestamp = pagePayload.timestamp;
      const direction = pagePayload.direction;
      var compareString = "";
    ...
    query += "ORDER BY timestamp " + orderBy + " LIMIT ?"

    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, [timestamp, pageSize] ,function (err, result, fields) {
          if (err){
            logger.error(err);
            reject(err)
          } else{
  ...
  selectTopTokens:  function(pageNumber, pageSize, pagePayload){
    var oThis = this;
    //logger.log("Getting  Recent Token Transactions FROM TOKEN TRANSACTION" );
    var query = "SELECT id, company_name, contract_address, company_symbol, price, market_cap, circulation from " + constants.BRANDED_TOKEN_TABLE_NAME +" where ";
...
// market_cap <= 45000000000000000000000 ORDER BY market_cap DESC LIMIT 2
    var marketCap
      ,orderBy
      ;
    if (pagePayload.market_cap !== undefined){
      marketCap = pagePayload.market_cap;
      const direction = pagePayload.direction;
      var compareString = "";
      ...
      query += "market_cap "+ compareString +" ? ";
    }else{
      marketCap = 0;
      query += "market_cap > ? ";
      orderBy = 'DESC'
    ...
    query += "ORDER BY market_cap " + orderBy +" LIMIT ?"

    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, [marketCap, pageSize] ,function (err, result, fields) {
          if (err){
            logger.error(err);
            reject(err)
          } else{

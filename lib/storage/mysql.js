...
MySQL.prototype = {
  ...
  selectAddressDetails: function (address){
    ...
    var query = "SELECT tokens, branded_token_id, total_transactions from  address WHERE address=? and branded_token_id != 0";
    return new Promise(function (resolve, reject) {
      try {
  ...
  selectChainData: function (){
    ...
    var query = "select MAX(id), SUM(token_holders), SUM(market_cap) from  " + constants.BRANDED_TOKEN_TABLE_NAME + ' where id!=0'
    return new Promise(function (resolve, reject) {

      logger.log(query);
      try {
        oThis.con.query(query, function (err, result) {
          if (err){
            logger.error(err);
            reject(err)
          } else{

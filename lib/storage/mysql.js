...
console.log("********\n\n\n\n\n\n\n\n insertion data :: ", dataArray);
  ...
  selectBrandedTokenTopUsers: function(brandedTokenId, topUsersCount){
    ...
    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, [brandedTokenId ,parseInt(topUsersCount)],function (err, result, fields) {
          if (err){
            logger.error(err);
            reject(err)
          } else{
...
// market_cap <= 45000000000000000000000 ORDER BY market_cap DESC LIMIT 2
  ...
  return new Promise(function (resolve, reject) {
    try {
      oThis.con.query(query, [marketCap,constants.BASE_CONTRACT_ADDRESS ,pageSize] ,function (err, result, fields) {
        if (err){
          logger.error(err);
          reject(err)
        } else{

...
MySQL.prototype = {
  ...
  selectAddressTransactions: function (tableName, address, pageSize, pagePayload) {
    ...
    var query = "SELECT * from " + tableName + " WHERE (address=? or corresponding_address=?)";
    ...
    if(!pagePayload){
      query += " order by timestamp desc,id desc limit ? ";
      whereValues = [address, address, pageSize]
    } else {
      // Timestamp and id are numbers in payload then only make a query
      if(!isNaN(parseInt(pagePayload.timestamp)) && !isNaN(parseInt(pagePayload.id))){
        const timestamp = pagePayload.timestamp
        ...
        query += " limit ?";
        whereValues = [address, address, timestamp, transactionId, pageSize]
      } else {
  ...
  selectAddressDetailsTotalTransactionCount: function(address){
    var oThis = this;
    var query = "SELECT COUNT(*) from " + constants.ADDRESS_TOKEN_TRANSACTION_TABLE_NAME + " WHERE address=? or corresponding_address=?";

    return new Promise(function (resolve, reject) {
      try {
        oThis.con.query(query, [address, address], function (err, result, fields) {
          if (err) {

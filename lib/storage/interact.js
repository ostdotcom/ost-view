...
DbHelper.prototype = {
  ...
  getTransaction: function (transactionHash) {
    ...
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectTransaction(constants.TRANSACTION_TABLE_NAME, transactionHash)
        .then(function (queryResponse) {
          queryResponse.forEach(function (element) {
            element.tokens = tokenUnits.convertToNormal(element.tokens).toString(10);
          });
   ...
   * @return {Promise}
   */
  getTokenTransaction: function (transactionHash) {
   ...
   * @return {Promise}
   */
  getBlockTokenTransactionNumber: function (blockNumber) {
    return this.dbObject.selectBlockTokenTransactionCountBlockNumber(blockNumber);
  },
  ...
  getContractLedger: function (contractAddress, pageSize, pagePayload) {
    ...
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectContractLedger(contractAddress, pageSize, pagePayload)
        .then(function (queryResponse) {
          queryResponse.forEach(function (element) {
            element.tokens = tokenUnits.convertToNormal(element.tokens).toString(10)
          })
   ...
   * @return {Promise}
   */
  getHighestInsertedBlock: function (min_block, max_block) {
    return this.dbObject.selectHighestInsertedBlock(min_block, max_block);
  },
  ...
  getLastVerifiedBlockTimestamp: function () {
    const oThis = this
      , blockObj = new BlockKlass()
    ;

    return new Promise(function(resolve, reject){
      blockObj.select('verified, max(timestamp) as max_timestamp, min(timestamp) as min_timestamp').group_by('verified').fire().then(function (rows) {
   ...
   * @param {Integer} pageSize - page size
   * @param {hash} pagePayload - payload for getting next or previous page data
   *
   * @return {Promise}
   */
  getBlockTransactionsFromBlockHash: function (blockHash, pageSize, pagePayload) {

    const oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.getBlockNumberFromBlockHash(blockHash)
        .then(function (response) {
          oThis.dbObject.getBlockTransactionsFromBlockNumber(response, pageSize, pagePayload)
            .then(function (queryResponse) {
              queryResponse.forEach(function (element) {
                element.tokens = tokenUnits.convertToNormal(element.tokens).toString(10);
              });
   ...
   * @return {Promise}
   */
  getBlockTransactionsFromBlockNumber: function (blockNumber, pageSize, pagePaylaod) {
    const oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectBlockTokenTransactionsFromBlockNumber(blockNumber, pageSize, pagePaylaod)
        .then(function (queryResponse) {
          queryResponse.forEach(function (element) {
            element.tokens = tokenUnits.convertToNormal(element.tokens).toString(10);
          });
  ...
  /**
   * To Delegate get block token transactions call to the DB
   *
   * @param {Integer} blockNumber - block number
   * @param {Integer} pageSize - page size
   * @param {hash} pagePayload - payload for getting next or previous page data
   ...
   * @return {Promise}
   */
  getBlockTokenTransactionsFromBlockNumber: function (blockNumber, pageSize, pagePaylaod) {
    const oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectBlockTokenTransactionsFromBlockNumber(blockNumber, pageSize, pagePaylaod)
        .then(function (queryResponse) {
  ...
  getAddressTokenTransactions: function (address, pageSize, pagePaylaod) {
    var oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectAddressTransactions(constants.ADDRESS_TOKEN_TRANSACTION_TABLE_NAME, address, pageSize, pagePaylaod)
        .then(function (queryResponse) {
          queryResponse.forEach(function (element) {
            element.tokens = tokenUnits.convertToNormal(element.tokens).toString(10);
          });
  ...
  getAddressDetailsTotalTransactionCount: function (address) {
    var oThis = this;
    return oThis.dbObject.selectAddressDetailsTotalTransactionCount(address);
  },
  ...
  getAddressDetailsFromDB: function (address) {
    var oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectAddressDetails(address)
        .then(function (queryResponse) {
          queryResponse.tokens = tokenUnits.convertToNormal(queryResponse.tokens).toString(10);
          resolve(queryResponse);
        })
        .catch(function (reason) {
          reject(reason);
  ...
  getAddressDetailsWithOutOST: function (address) {
    var oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectAddressDetailsWithOutOST(address)
        .then(function (queryResponse) {
  ...
  /**
   * To Delegate insert blocks call to the DB
   *
   * @param {Array} blockDataArray - Array of Block Data Content Array
   *
   * @return {Promise}
   */
  insertBlocks: function (blockDataArray) {
    return this.dbObject.insertBlockDataArray(constants.BLOCK_TABLE_NAME, constants.BLOCKS_DATA_SEQUENCE, blockDataArray);
  },
  ...
  insertTransaction: function (transactionDataArray) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      logger.info("Inserting total records in transactions ", transactionDataArray.length);
      var transactionResponse = oThis.dbObject.insertDataForTransactionHashTables(
        constants.TRANSACTION_TABLE_NAME, constants.TRANSACTION_DATA_SEQUENCE, transactionDataArray);
      ...
      logger.info("Inserting total records in address_transactions ", addressTransactionData.length);

      transactionResponse
        .then(function (res) {
          result.push(res);
          logger.info("Inserted total records in transactions ", res.length);

          oThis.insertAddressTransaction(addressTransactionData)
            .then(function (res) {
              result.push(res);
              logger.info("Inserted total records in address_transactions ", res.length);
              resolve(result);
            });
        });
    });
  ...
  insertAddressTransaction: function (addressTransactionData) {
    return this.dbObject.insertDataForBlockFetcher(constants.ADDRESS_TRANSACTION_TABLE_NAME, constants.ADDRESS_TRANSACTION_DATA_SEQUENCE, addressTransactionData);
  },
  ...
  insertTokenTransaction: function (tokenTransactionDataArray) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var p = oThis.dbObject.insertDataForIdTables(constants.TOKEN_TRANSACTION_TABLE_NAME, constants.TOKEN_TRANSACTION_DATA_SEQUENCE,
        tokenTransactionDataArray);

      for (var ind in tokenTransactionDataArray) {
        //Format token transactions
        var txnArray = oThis.formatAddressTokenTransactionData(tokenTransactionDataArray[ind]);
        txnArray.forEach(function (addrTxn) {
          addressTransactionData.push(addrTxn);
        });
      }

      logger.info("Inserting total records in token_transactions ", tokenTransactionDataArray.length);
      logger.info("Inserting total records in address_token_transactions ", addressTransactionData.length);
      //logger.debug(addressTransactionData);

      p.then(function (res1) {
        var res = res1;
        result.push(res);
        logger.info("Inserted total records in token_transactions ", res.length);

        oThis.insertAddressTokenTransaction(addressTransactionData)
          .then(function (res) {
            result.push(res);
            logger.info("Inserted total records in address_token_transactions ", res.length);
            resolve(result);
          });
      });
    });
  ...
  insertAddressTokenTransaction: function (addressTokenTransactionData) {
    return this.dbObject.bulkInsert(constants.ADDRESS_TOKEN_TRANSACTION_TABLE_NAME, constants.ADDRESS_TOKEN_TRANSACTION_DATA_SEQUENCE, addressTokenTransactionData);
  },
   ...
   * @return {Promise}
   */
  updateVerifiedFlag: function (blockNumber, verifiedVal) {
    if (undefined != blockNumber) {
      return this.dbObject.updateAttribute(constants.BLOCK_TABLE_NAME, 'verified', verifiedVal, 'block_number', blockNumber);
    }
    return Promise.reject(new Error('blockNumber is undefined'));
  },
  ...
  insertIntoAggregateTable: function (aggregateDataArray) {
    return this.dbObject.insertDataForIdTables(constants.AGGREGATE_TABLE_NAME, constants.AGGREGATE_DATA_SEQUENCE, aggregateDataArray);
  },
   ...
   * @param {Integer} timeId2 - right bound of timestamp
   */
  getTransferAggregateDataWithinTimestamp: function (contractId, timeId1, timeId2) {
    return this.dbObject.selectTransferAggregateDataWithinTimestamp(contractId, timeId1, timeId2, false);
  },
  ...
  /**
   * Get transfers and volume of transactions in given contract address
   * @param {Integer} contractAddress - contract address
   */
  getTransfersAndVolumesOfBrandedTokenTransactions: function (contractAddress) {
    return this.dbObject.selectTransfersAndVolumesOfBrandedTokenTransactions(contractAddress);
  },
   ...
   */
  getBrandedTokenTopUsers: async function (contractId, topUsersCount) {

    var queryResponse = await this.dbObject.selectBrandedTokenTopUsers(contractId, topUsersCount);
    for (var i = 0; i < queryResponse.length; i++) {
      queryResponse[i].tokens = tokenUnits.convertToNormal(queryResponse[i].tokens).toString(10);
      queryResponse[i].tokens_earned = tokenUnits.convertToNormal(queryResponse[i].tokens_earned).toString(10);
      queryResponse[i].tokens_spent = tokenUnits.convertToNormal(queryResponse[i].tokens_spent).toString(10);
    }
  ...
  getAddressesWithBrandedToken: function (brandedTokenId, PageSize, pagePaylaod) {
    var oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectAddressesWithBrandedTokenId(brandedTokenId, PageSize, pagePaylaod)
        .then(function (queryResponse) {

          queryResponse.forEach(function (element) {

            element.tokens = tokenUnits.convertToNormal(element.tokens).toString(10);
            element.tokens_earned = tokenUnits.convertToNormal(element.tokens_earned).toString(10);
            element.tokens_spent = tokenUnits.convertToNormal(element.tokens_spent).toString(10);
  ...
  getRecentTokenTransactions: function (pageSize, pagePaylaod) {
    var oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectRecentTokenTransactions(pageSize, pagePaylaod)
        .then(function (queryResponse) {
  ...
  getTopTokens: function (pageSize, pagePayload) {
    var oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectTopTokens(pageSize, pagePayload)
        .then(function (queryResponse) {

          queryResponse.forEach(function (element) {

            element.market_cap = tokenUnits.convertToNormal(element.market_cap).toString(10);
            element.circulation = tokenUnits.convertToNormal(element.circulation).toString(10);
            element.token_ost_volume = tokenUnits.convertToNormal(element.token_ost_volume).toString(10);
          });
          resolve(queryResponse);
        })
        .catch(function (reason) {
   ...
   */
  getChainStatsData: function () {
    return this.dbObject.selectTokenStatsData(0);
  },
   ...
   * @returns {*|wrappedPromise|null}
   */
  getTokenStatsData: function (brandedTokenId) {
    return this.dbObject.selectTokenStatsData(brandedTokenId);
  },

  /**
   * To get Type Token Data
   * @param brandedTokenId Branded Token Id
   * @returns {*|wrappedPromise|null}
   */
  getTypeTokenStatsData: function (brandedTokenId) {
    return this.dbObject.selectTransferTokenStatsData(brandedTokenId);
  },

  /**
   * To get all the block transactions
   * @param blockNumber blockNumber
   * @returns {*|Promise}
   */
  getBlockTransactionFromBlockNumber: function (blockNumber) {
    return this.dbObject.selectBlockTransactionsByBlockNumber(blockNumber);
  },

  /**
   * To get all the block transactions
   * @param btNameOrSymbol name of branded token or symbol
   * @returns {*|Promise}
   */
  getContractAddressFromBrandedTokenNameOrSymbol: function (btNameOrSymbol) {
    return this.dbObject.selectContractAddressFromBrandedTokenNameOrSymbol(btNameOrSymbol);
  },

  /**
   * TO check block Insertion
   * @param blockNumber
   */
  isBlockInserted: function (blockNumber) {
    var oThis = this;
    return new Promise(function (resolve, reject) {
      oThis.dbObject.selectBlockFromBlockNumber(blockNumber)
        .then(function (block) {
          return resolve(block !== undefined);
        })
        .catch(function (err) {
          logger.error(err);
...
const dbHelperHandler = (function () {
  ...
  return {
    getInstance: function (chainId) {
      const dbConfig = core_config.getChainConfig(chainId) && core_config.getChainConfig(chainId).db_config;
      if (!dbConfig) {
        logger.error("interact :: getInstance :: dbconfig is not defined for chain id", chainId);
        process.exit(1);
      }
      if (!dbHelpers[chainId]) {
        const instance = createInstance(dbConfig);
        dbHelpers[chainId] = instance
      }
      return dbHelpers[chainId];
    }
  };

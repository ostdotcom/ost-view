  ...
  getAddressDetailsFromDB: function (address){
    ...
    return new Promise(function(resolve, reject){
      oThis.dbObject.selectAddressDetails(address)
        .then(function(queryResponse){
          queryResponse.tokens = tokenUnits.convertToNormal(queryResponse.tokens);
          resolve(queryResponse);
        })
        .catch(function(reason){
          reject(reason);
      ...
      oThis.dbObject.selectAddressesWithBrandedTokenId(brandedTokenId, PageSize, pagePaylaod)
        .then(function(queryResponse){
          ...
          queryResponse.forEach(function(element){
            ...
            element.tokens = tokenUnits.convertToNormal(element.tokens);
            element.tokens_earned = tokenUnits.convertToNormal(element.tokens_earned);
            element.tokens_spent = tokenUnits.convertToNormal(element.tokens_spent);
      ...
      oThis.dbObject.selectTopTokens(pageSize, pagePayload)
        .then(function(queryResponse){
          ...
          queryResponse.forEach(function(element){
            ...
            element.market_cap = tokenUnits.convertToNormal(element.market_cap).toFormat(18);
            element.circulation = tokenUnits.convertToNormal(element.circulation);

...
DbHelper.prototype = {
  ...
  getBrandedTokenTopUsers: async function(contractId, topUsersCount){
    ...
    for(var i=0; i<queryResponse.length; i++){
      queryResponse[i].tokens = tokenUnits.convertToNormal(queryResponse[i].tokens);
      queryResponse[i].tokens_earned = tokenUnits.convertToNormal(queryResponse[i].tokens_earned);
      queryResponse[i].tokens_spent = tokenUnits.convertToNormal(queryResponse[i].tokens_spent);
    }

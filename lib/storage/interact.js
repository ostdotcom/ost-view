...
DbHelper.prototype = {
  ...
  getLastInsertedTimeStamp: function () {
    return this.dbObject.selectLastInsertedTimeStamp();
  },
  /**
   * To calculate getLastAggregatedBlockNumber call to the DB
   *
   * @return {Promise}
   */
  getLastAggregatedBlockNumber: function () {
    var oThis = this;
    return new Promise(function(resolve, reject) {
        oThis.getLastInsertedTimeStamp()
            .then(function( res ){
                oThis.getBlockNumberForTimestamp(+res + constants.AGGREGATE_CONSTANT)
                    .then(function(res){
                        resolve(res);
                    }, reject);
            }, reject);

...
function Result(data, errCode, errMsg) {
  ...
  this.renderResponse = function (res, status, contentType) {
    if (contentType != 'application/json') {
      logger.log("#renderResponse resultType",this.data,constants[this.data.result_type]);
      return res.render(constants[this.data.result_type], this.data[this.data.result_type]);
    }
    status = status || 200;

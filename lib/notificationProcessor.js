...
NotificationProcessor.prototype.processTransaction = async function (msgPayload) {
  ...
  const addressObj = new AddressKlass(chainId)
    ...
    , addressTypeVal = addressObj.invertedAddressTypes[addressConst.contractAddressType]
  ;
  ...
  if (addressRow[0]){
    ...
    // if addressTypeVal not correct , updATE
  }else{
...
NotificationProcessor.prototype.processBrandedTokenEvent = async function (msgPayload) {
  ...
  if (!brandedTokenRowHash) {
    ...
    return await insertBrandedTokenObj.insert(brandedTokenRowHash, {insertWithIgnore: true}).fire();
  } else {
    ...
    const insertBrandedTokenObj = new BrandedTokenKlass(chainId);
    return await insertBrandedTokenObj.update(updateFields).where({id: brandedTokenRowHash.id}).fire();
  }
